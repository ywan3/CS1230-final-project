...
- To read in the shapes, I created a `shape` class, with the 4 types of shapes in our project as subclasses. 
  - This class has memeber functions to generate the vertices data points and normals for the tessellated shapes. 

#### VAO and VBO
- To initialize VAO, VBO and populate them correctly, I wrote a helper function `uppdateVAOVBO()` to help me do so.
  - For each type of shape, I use one unique VBO. Since shapes of the same type are essentially the same exact shape in their object space, using at most 4 VBOs ensure the efficiency of the code. 
  - Since all our data use the same format (positions and normals), I use only one VAO to intepret all my VBOs. 
  - This function is called whenever we read in a new scene file or settings changed. 
  
#### paintGL()
- In the function `paintGL()`, I loop over all the lights in the scene, and then pass them to the shader as uniforms. 
  - I also pass the total number of lights to avoid scene getting brigher and brighter as we read in more files
  - I then pass the camera positon as a uniform
- Then, I loop through all the shapes again, and bind the corresponding VAO and VBO. 
  - in the loop, I passed variables that are specific to that shape (ctm, material, etc.) as uniforms.
- Eventually, I use the function `glDrawArrays()` to draw the triangles onto our canvas.

### Collaboration/References

### Known Bugs
- My cone normals are wrong!! (Spent so long trying to fix it :<) This is why in a lot of the outputs my cones are off. 
